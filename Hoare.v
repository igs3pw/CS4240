...
(**
 * as1: In the given state, the variable X is 3.
 * as2: In the given state, the variable X is less than or equal to the variable Y.
 * as3: In the given state, the variable X is 3 or X is less then or equal to Y.
 * as4: In the given state, Z squared is less than or equal to X and it is not true
 *      that the successor of Z squared is less than or equal to X.
 * as5: The given state is always valid.
 * as6: The given state is always invalid.
 *)
...
(** Paraphrase the following Hoare triples in English.
  ...
  After executing the command c, it follows that X is equal to 5.

   2) {{X = m}} c {{X = m + 5)}}

  Given that X is equal to m, after executing c, it follows that X is five greater than m.

   3) {{X <= Y}} c {{Y <= X}}

  Given that X is less than or equal to Y, after executing c, it follows that Y is less than or equal to X.

   4) {{True}} c {{False}}

  The statement c will never finish executing.

   5) {{X = m}} 
      c
      {{Y = real_fact m}}.

  Given that X is equal to m, after executing c, it follows that Y is equal to the result of real_fact executed on m.

   6) {{True}} 
      c 
      {{(Z * Z) <= m /\ ~ (((S Z) * (S Z)) <= m)}}

  When c is executed, it follows that Z squared is less than or equal to m
    and that the successor of Z squared is not less than or equal to m.
...
(** Which of the following Hoare triples are _valid_ -- i.e., the
  ...
  True. The precondition is unreachable, so anything follows.

   7) {{True}} WHILE True DO SKIP END {{False}}

  True. The loop never terminates.
  ...
  False. The loop never terminates.
...
(* a = X * X, which is ok in assignment, but would fail in the post-condition
 * With proper hoare logic, it would also fail in the pre-condition *)
...
Proof.
  ...
  (*intros st st' HX HP.
  inversion HX; inversion HP; subst.
  unfold update. simpl.*)

  split. 
    inversion H. subst. inversion H0. subst.
    unfold update.
    rewrite functional_extensionality with (g:=st).
      assumption.
    ...
    Case "st X = aeval (update st X m) a".
    inversion H. subst. inversion H0. subst.
    rewrite update_eq.
    rewrite functional_extensionality with (f:=(update (update st X (aeval st a)) X (st X))) (g:=st).
      trivial.

      intros x.
      unfold update. destruct (eq_id_dec X x).
        subst. trivial.
...
Proof.
  ...
  inversion HX. subst. rewrite update_eq. exists (st X).
  rewrite functional_extensionality with (f:=(update (update st X (aeval st a)) X (st X))) (g:=st).
    split.
      assumption.
    ...
    intros x. unfold update. destruct (eq_id_dec X x).
      subst. trivial.
...
Proof.
  ...
  Case "Left".
    eapply hoare_consequence_pre.
      ...
      unfold assn_sub. simpl. unfold update. simpl.
      split; reflexivity.
  Qed.
(** [] *)
...
Proof.
  ...
  Case "Left".
    eapply hoare_consequence_pre.
      ...
      unfold assn_sub. simpl. unfold update. simpl. intros st H. apply H.
  Qed.
(** [] *)
...
(**
  You cannot prove it as the value of 'a' may be dependent on variable X.
...
Lemma hoare_repeat : forall P Q b c,
  {{P}} c {{fun st => (P st /\ ~(bassn b st)) \/ (Q st /\ bassn b st)}} ->
...
Proof.
  ...
  Case "E_RepeatEnd".
    unfold hoare_triple in Hhoare.
      assert ((P st' /\ ~ bassn b st' \/ Q st' /\ bassn b st') -> (Q st' /\ bassn b st')).
        intros Hfull. inversion Hfull.
          inversion H0. unfold bassn in H2. rewrite H in H2.
          apply ex_falso_quodlibet. apply H2. reflexivity.

          assumption.
      apply H0. eapply Hhoare.
        apply He.
  ...
  Case "E_RepeatLoop".
    apply IHHe2.
      trivial.
    unfold hoare_triple in Hhoare.
      assert ((P st' /\ ~ bassn b st' \/ Q st' /\ bassn b st') -> P st').
        intros Hfull. inversion Hfull.
          inversion H0. assumption.

          inversion H0. unfold bassn in H2. rewrite H in H2. inversion H2.
      apply H0. eapply Hhoare.
        apply He1.
...
(*Lemma hoare_repeat : forall P b c,
  {{P}} c {{P}} ->
  {{P}} REPEAT c UNTIL b END {{fun st => P st /\ bassn b st}}. Admitted.
Proof.
...
Proof.
  eapply hoare_consequence_post.
    apply hoare_repeat with (Q:=(fun st => st X = 0 /\ st Y > 0)).
      eapply hoare_seq. 
        apply hoare_asgn.
        ...
        eapply hoare_consequence_pre.
          ...
          intros st H. unfold bassn, assn_sub, update. simpl.
          right. admit. (* FIXME *)
    intros st H. inversion H. assumption.
  Qed.
...
(* Since the command is HAVOC, Q must hold for all natural numbers,
 * thus P must hold for all natural numbers as well. *)
