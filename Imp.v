...
Proof.
  ...
  split.
    ...
    induction e; simpl; intros; subst; constructor;
      try apply IHe1; try apply IHe2; reflexivity.
  Qed.
(** [] *)
End AExp.
...
Proof.
  ...
  destruct (eq_id_dec x1 x3).
    ...
    destruct (eq_id_dec x2 x3); reflexivity.
  Qed.
(** [] *)
...
Theorem XtimesYinZ_spec : forall st x y st',
  st X = x -> st Y = y ->
  XtimesYinZ / st || st' ->
  st' X = x -> st' Y = y -> st' Z = x * y.
Proof.
  intros st x y st' HX HY Heval HX' HY'.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | R_Ass: forall x a, no_whilesR (x ::= a)
  | R_Seq: forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1 ;; c2)
  | R_If: forall b c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (IFB b THEN c1 ELSE c2 FI).
...
Proof.
  intros c.
  split.
  induction c; intros H; inversion H.
    apply R_Skip.

    apply R_Ass.

    apply R_Seq.
      apply IHc1. destruct (no_whiles c1); try reflexivity; try (inversion H1).

      apply IHc2. destruct (no_whiles c1); destruct (no_whiles c2); try reflexivity; try (inversion H1).
  ...
  Case "seq".
    simpl. intros H. inversion H. rewrite IHc1. rewrite IHc2. reflexivity.
      apply H3.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | SPlus :: t, n1::n2::t' => s_execute st ((n2 + n1) :: t') t
  | SMinus :: t, n1::n2::t' => s_execute st ((n2 - n1) :: t') t
  | SMult :: t, n1::n2::t' => s_execute st ((n2 * n1) :: t') t
  | h :: t, _ => s_execute st stack t
  end.
...
Fixpoint s_compile (e : aexp) : list sinstr :=
  ...
  | AId id =>
      SLoad id :: nil
  | APlus e1 e2 =>
      (s_compile e1) ++ (s_compile e2) ++ SPlus :: nil
  | AMinus e1 e2 =>
      (s_compile e1) ++ (s_compile e2) ++ SMinus :: nil
  | AMult e1 e2 =>
      (s_compile e1) ++ (s_compile e2) ++ SMult :: nil
  end.
